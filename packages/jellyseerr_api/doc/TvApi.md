...
final api = JellyseerrApi().getTvApi();
...
final api = JellyseerrApi().getTvApi();
final num tvId = 76479; // num | 
final num seasonNumber = 123456; // num | 
final String language = en; // String | 

try {
    final response = api.tvTvIdSeasonSeasonNumberGet(tvId, seasonNumber, language);
    print(response);
} catch on DioException (e) {
    print('Exception when calling TvApi->tvTvIdSeasonSeasonNumberGet: $e\n');
}
```
